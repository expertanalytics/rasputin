...
| 12 | DEM to elevated mesh: the zero-copy `RasterView` and its binding, z sampled per mesh vertex, `rasputin mesh --dem file.tif` | designed | `docs/increments/12-dem-to-mesh.md` |
...
and 3 and the first half of 5 are designed as increment 12; gap 2 has no record
yet.
...
1. **Raster ingestion, Python side.** The C++ `raster/` module samples; nothing
   ...
   `RasterView` and the pybind11 buffer surface are increment 12, designed.
...
4. **Mesh output.** Shipped as increment 10 (PLY, for QGIS) and extended by
   ...
   feature names. Real z is gap 3, designed in increment 12.
