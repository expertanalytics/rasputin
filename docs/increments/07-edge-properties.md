...
alongside CRS metadata and everything else `project_structure.md:206-210` keeps out
...
2. **`fingerprint()`, a stable digest over the sorted `(bit, name)` pairs**, is
   ...
   **exactly as CRS is**. An artifact read back with a fingerprint that does not
   ...
   ruling `project_structure.md:206-210` already makes about CRS, for the same
