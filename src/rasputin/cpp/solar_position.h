...
inline auto azimuth_and_elevation() {
    return [] (
        double e0, double Gamma, double Phi, const double alpha_mark, const double delta_mark, const double H_mark
    ) {
...
inline auto parabolic_dt_calc() {
    return [] (const double year) {
         return -20 + 32*pow((year - 1820)/20.0, 2);
    };
}

inline auto coarse_date_calc() {
    return [] (
...
inline auto coarse_timestamp_calc() {
    using namespace std::chrono;

    auto ep = sys_days{1970y / January / 1};
...
inline double limit_degrees(const double degrees) {
    return fmod(fmod(degrees, 360) + 360, 360);
}

inline double d2r(const double d) {
    return d*M_PI/180.0;
}

inline double r2d(const double r) {
    return r*180.0/M_PI;
}

inline double jd_from_cal(unsigned int year, unsigned int month, const double day){
    ...
    // Equation (4)
    const double jd_temp = trunc(365.25*(year + 4716)) + trunc(30.6001*(month + 1)) + day - 1524.5;
    if (jd_temp < 2299160)
        return jd_temp;
    const double A = trunc(year/100.);
    return jd_temp + 2 - A + trunc(A/4);
}

inline double jd_from_clock(const std::chrono::system_clock::time_point tp) {
    using namespace std::chrono;

    const auto dt = tp - sys_days{1970y / January / 1};
    const long t = duration_cast<seconds>(dt).count();

    return t/86400.0 + 2440587.5;
}

inline double jde(const double julian_day, const double dt){
    return julian_day + dt/86400.0;
}

inline double jc(const double julian_day) {
    return (julian_day - 2451545.0)/36525.0;
}

inline double jce(const double julian_ephemeris_day) {
    return (julian_ephemeris_day - 2451545.0)/36525.0;
}

inline double jme(const double julian_ephemeris_centry) {
...
inline double geocentric_longitude(const double heliocentric_longitude) {
    return heliocentric_longitude + 180;
}

inline double geocentric_latitude(const double heliocentric_latitude) {
    return -heliocentric_latitude;
}


inline std::pair<double, double> nutation(const double julian_ephemeris_centry) {
    const double jce = julian_ephemeris_centry;
    const double jce2 = jce*jce;
    const double jce3 = jce2*jce;

    const double X0 = 297.85036 + 445267.111480*jce - 0.0019142*jce2 + jce3/189474;
    const double X1 = 357.52772 +  35999.050340*jce - 0.0001603*jce2 - jce3/300000;
    const double X2 = 134.96298 + 477198.867398*jce + 0.0086972*jce2 + jce3/56250;
    const double X3 = 93.27191  + 483202.017538*jce - 0.0036825*jce2 + jce3/327270;
    const double X4 = 125.04452 -   1934.136261*jce + 0.0020708*jce2 + jce3/450000;

    const double dksi_00 = (-171996 - 174.2*jce)*sin(d2r(                        X4  ));
    const double dksi_01 = ( -13187 -   1.6*jce)*sin(d2r(-X0*2            +X3*2 +X4*2));
    const double dksi_02 = (  -2274 -   0.2*jce)*sin(d2r(                  X3*2 +X4*2));
    const double dksi_03 = (   2062 +   0.2*jce)*sin(d2r(                        X4*2));
    const double dksi_04 = (   1426 -   3.4*jce)*sin(d2r(       X1                   ));
    const double dksi_05 = (    712 +   0.1*jce)*sin(d2r(            X2              ));
    const double dksi_06 = (   -517 +   1.2*jce)*sin(d2r(-X0*2 +X1        +X3*2 +X4*2));
    const double dksi_07 = (   -386 -   0.4*jce)*sin(d2r(                  X3*2 +X4  ));
    const double dksi_08 =     -301             *sin(d2r(            X2   +X3*2 +X4*2));
    const double dksi_09 = (    217 -   0.5*jce)*sin(d2r(-X0*2 -X1        +X3*2 +X4*2));
    const double dksi_10 =     -158             *sin(d2r(-X0*2      +X2              ));
    const double dksi_11 = (    129 +   0.1*jce)*sin(d2r(-X0*2            +X3*2 +X4  ));
    const double dksi_12 =      123             *sin(d2r(           -X2   +X3*2 +X4*2));
    const double dksi_13 =       63             *sin(d2r( X0*2                       ));
    const double dksi_14 = (     63 +   0.1*jce)*sin(d2r(            X2         +X4  ));
    const double dksi_15 =      -59             *sin(d2r( X0*2      -X2   +X3*2 +X4*2));
    const double dksi_16 = (    -58 -   0.1*jce)*sin(d2r(           -X2         +X4  ));
    const double dksi_17 =      -51             *sin(d2r(            X2   +X3*2 +X4  ));
    const double dksi_18 =       48             *sin(d2r(-X0*2      +X2*2            ));
    const double dksi_19 =       46             *sin(d2r(           -X2*2 +X3*2 +X4  ));
    const double dksi_20 =      -38             *sin(d2r( X0*2            +X3*2 +X4*2));
    const double dksi_21 =      -31             *sin(d2r(            X2*2 +X3*2 +X4*2));
    const double dksi_22 =       29             *sin(d2r(            X2*2            ));
    const double dksi_23 =       29             *sin(d2r(-X0*2      +X2   +X3*2 +X4*2));
    const double dksi_24 =       26             *sin(d2r(                  X3*2      ));
    const double dksi_25 =      -22             *sin(d2r(-X0*2            +X3*2      ));
    const double dksi_26 =       21             *sin(d2r(           -X2   +X3*2 +X4  ));
    const double dksi_27 = (     17 -   0.1*jce)*sin(d2r(       X1*2                 ));
    const double dksi_28 =       16             *sin(d2r( X0*2      -X2         +X4  ));
    const double dksi_29 = (    -16 +   0.1*jce)*sin(d2r(-X0*2 +X1*2      +X3*2 +X4*2));
    const double dksi_30 =      -15             *sin(d2r(       X1              +X4  ));
    const double dksi_31 =      -13             *sin(d2r(-X0*2      +X2         +X4  ));
    const double dksi_32 =      -12             *sin(d2r(      -X1              +X4  ));
    const double dksi_33 =       11             *sin(d2r(            X2*2 -X3*2      ));
    const double dksi_34 =      -10             *sin(d2r( X0*2      -X2   +X3*2 +X4  ));
    const double dksi_35 =       -8             *sin(d2r( X0*2      +X2   +X3*2 +X4*2));
    const double dksi_36 =        7             *sin(d2r(       X1        +X3*2 +X4*2));
    const double dksi_37 =       -7             *sin(d2r(-X0*2 +X1  +X2              ));
    const double dksi_38 =       -7             *sin(d2r(      -X1        +X3*2 +X4*2));
    const double dksi_39 =       -7             *sin(d2r( X0*2            +X3*2 +X4  ));
    const double dksi_40 =        6             *sin(d2r( X0*2      +X2              ));
    const double dksi_41 =        6             *sin(d2r(-X0*2      +X2*2 +X3*2 +X4*2));
    const double dksi_42 =        6             *sin(d2r(-X0*2      +X2   +X3*2 +X4  ));
    const double dksi_43 =       -6             *sin(d2r( X0*2      -X2*2       +X4  ));
    const double dksi_44 =       -6             *sin(d2r( X0*2                  +X4  ));
    const double dksi_45 =        5             *sin(d2r(      -X1  +X2              ));
    const double dksi_46 =       -5             *sin(d2r(-X0*2 -X1        +X3*2 +X4  ));
    const double dksi_47 =       -5             *sin(d2r(-X0*2                  +X4  ));
    const double dksi_48 =       -5             *sin(d2r(            X2*2 +X3*2 +X4  ));
    const double dksi_49 =        4             *sin(d2r(-X0*2      +X2*2       +X4  ));
    const double dksi_50 =        4             *sin(d2r(-X0*2 +X1        +X3*2 +X4  ));
    const double dksi_51 =        4             *sin(d2r(            X2   -X3*2      ));
    const double dksi_52 =       -4             *sin(d2r(-X0        +X2              ));
    const double dksi_53 =       -4             *sin(d2r(-X0*2 +X1                   ));
    const double dksi_54 =       -4             *sin(d2r( X0                         ));
    const double dksi_55 =        3             *sin(d2r(            X2   +X3*2      ));
    const double dksi_56 =       -3             *sin(d2r(           -X2*2 +X3*2 +X4*2));
    const double dksi_57 =       -3             *sin(d2r(-X0   -X1  +X2              ));
    const double dksi_58 =       -3             *sin(d2r(       X1  +X2              ));
    const double dksi_59 =       -3             *sin(d2r(      -X1  +X2   +X3*2 +X4*2));
    const double dksi_60 =       -3             *sin(d2r( X0*2 -X1  -X2   +X3*2 +X4*2));
    const double dksi_61 =       -3             *sin(d2r(            X2*3 +X3*2 +X4*2));
    const double dksi_62 =       -3             *sin(d2r( X0*2 -X1        +X3*2 +X4*2));

    const double deps_00 = (92025 + 8.9*jce)*cos(d2r(                          X4  ));
    const double deps_01 = ( 5736 - 3.1*jce)*cos(d2r(-X0*2              +X3*2 +X4*2));
    const double deps_02 = (  977 - 0.5*jce)*cos(d2r(                    X3*2 +X4*2));
    const double deps_03 = ( -895 + 0.5*jce)*cos(d2r(                          X4*2));
    const double deps_04 = (   54 - 0.1*jce)*cos(d2r(       X1                     ));
    const double deps_05 =     -7           *cos(d2r(              X2              ));
    const double deps_06 = (  224 - 0.6*jce)*cos(d2r(-X0*2 +X1          +X3*2 +X4*2));
    const double deps_07 =    200           *cos(d2r(                    X3*2 +X4  ));
    const double deps_08 = (  129 - 0.1*jce)*cos(d2r(              X2   +X3*2 +X4*2));
    const double deps_09 = (  -95 + 0.3*jce)*cos(d2r(-X0*2 -X1          +X3*2 +X4*2));
    const double deps_10 =    -70           *cos(d2r(-X0*2              +X3*2 +X4  ));
    const double deps_11 =    -53           *cos(d2r(             -X2   +X3*2 +X4*2));
    const double deps_12 =    -33           *cos(d2r(              X2         +X4  ));
    const double deps_13 =     26           *cos(d2r( X0*2        -X2   +X3*2 +X4*2));
    const double deps_14 =     32           *cos(d2r(             -X2         +X4  ));
    const double deps_15 =     27           *cos(d2r(              X2   +X3*2 +X4  ));
    const double deps_16 =    -24           *cos(d2r(             -X2*2 +X3*2 +X4  ));
    const double deps_17 =     16           *cos(d2r( X0*2              +X3*2 +X4*2));
    const double deps_18 =     13           *cos(d2r(              X2*2 +X3*2 +X4*2));
    const double deps_19 =    -12           *cos(d2r(-X0*2        +X2   +X3*2 +X4*2));
    const double deps_20 =    -10           *cos(d2r(             -X2   +X3*2 +X4  ));
    const double deps_21 =     -8           *cos(d2r( X0*2        -X2         +X4  ));
    const double deps_22 =      7           *cos(d2r(-X0*2 +X1*2        +X3*2 +X4*2));
    const double deps_23 =      9           *cos(d2r(       X1                +X4  ));
    const double deps_24 =      7           *cos(d2r(-X0*2        +X2         +X4  ));
    const double deps_25 =      6           *cos(d2r(      -X1                +X4  ));
    const double deps_26 =      5           *cos(d2r( X0*2        -X2   +X3*2 +X4  ));
    const double deps_27 =      3           *cos(d2r( X0*2        +X2   +X3*2 +X4*2));
    const double deps_28 =     -3           *cos(d2r(       X1          +X3*2 +X4*2));
    const double deps_29 =      3           *cos(d2r(      -X1          +X3*2 +X4*2));
    const double deps_30 =      3           *cos(d2r( X0*2              +X3*2 +X4  ));
    const double deps_31 =     -3           *cos(d2r(-X0*2        +X2*2 +X3*2 +X4*2));
    const double deps_32 =     -3           *cos(d2r(-X0*2        +X2   +X3*2 +X4  ));
    const double deps_33 =      3           *cos(d2r( X0*2        -X2*2       +X4  ));
    const double deps_34 =      3           *cos(d2r( X0*2                    +X4  ));
    const double deps_35 =      3           *cos(d2r(-X0*2 -X1          +X3*2 +X4  ));
    const double deps_36 =      3           *cos(d2r(-X0*2                    +X4  ));
    const double deps_37 =      3           *cos(d2r(              X2*2 +X3*2 +X4  ));

    const double dksi = (dksi_00 + dksi_01 + dksi_02 + dksi_03 + dksi_04 + dksi_05 + dksi_06 + dksi_07 + dksi_08 + dksi_09
    ...
    const double deps = (deps_00 + deps_01 + deps_02 + deps_03 + deps_04 + deps_05 + deps_06 + deps_07 + deps_08 + deps_09
...
inline double true_ecliptic_obliquity(const double julian_ephemeris_millennium, const double obliquity_nutation) {
    ...
    // Equation (24)
    const double eps_0 = 84381.448 - 4680.93*U   - 1.55*U2 + 1999.25*U3
...
inline double aberration_correction(const double R) {
    // Equation (26)
    return -20.4898/(3600.0*R);
}

inline double apparent_sun_longitude(const double geoc_long, const double nut_long, const double ab_corr) {
    // Equation (27)
    return geoc_long + nut_long + ab_corr;
}

inline double apparent_Greenwich_sidereal_time(
    ...
    const double nu0 =
        ...
        + 0.000387933*pow(julian_ephemeris_centry, 2)
...
inline double geocentric_sun_right_ascension(
    const double app_sun_longitude, const double true_ecliptic_obliq, const double geocentric_lat
) {
    // Equation (30)
    const double lambda_rad = d2r(app_sun_longitude);
    const double eps_rad = d2r(true_ecliptic_obliq);
    const double beta_rad = d2r(geocentric_lat);
    const double alpha = atan2(sin(lambda_rad)*cos(eps_rad) - tan(beta_rad)*sin(eps_rad), cos(lambda_rad));

    return limit_degrees(r2d(alpha));
}

inline double geocentric_sun_declination(
    const double app_sun_longitude, const double true_ecliptic_obliq, const double geocentric_lat
) {
    // Equation (31)
    const double lambda_rad = d2r(app_sun_longitude);
    const double eps_rad = d2r(true_ecliptic_obliq);
    const double beta_rad = d2r(geocentric_lat);
    const double delta = asin(sin(beta_rad)*cos(eps_rad) + cos(beta_rad)*sin(eps_rad)*sin(lambda_rad));

    return r2d(delta);
}

inline double observer_local_hour_angle(
    const double app_Greenwich_sidereal_time, const double geocentric_sun_right_asc, const double geographical_lon
) {
    // Equation (32)
...
inline std::tuple<double, double, double> topocentric_values(
    const double R,
...
inline double uncorrected_topocentric_elevation_angle(
    ...
    const double eps_0 = r2d(asin(sin(lat_rad)*sin(sun_decl_rad) + cos(lat_rad)*cos(sun_decl_rad)*cos(hour_angle_rad)));

    return eps_0;
}

inline std::pair<double, double> topocentric_zenith_angle(const double eps_0, const double P, const double T) {
    // Equation (42)
...
inline std::pair<double, double> topocentric_astronomers_and_azimuth_angle(
    const double topo_local_hour_angle, const double geographical_latitude, const double topo_sun_declination
) {
    // Equation (45)
    const double h_rad = d2r(topo_local_hour_angle);
    const double lat_rad = d2r(geographical_latitude);
    const double sun_decl_rad = d2r(topo_sun_declination);
    const double Gamma = limit_degrees(
        r2d(atan2(sin(h_rad), cos(h_rad)*sin(lat_rad) - tan(sun_decl_rad)*cos(lat_rad)))
    );
    // Equation (46)
...
inline double indicence_angle(
    const double topo_zenith_angle,
    const double surface_slope,
    const double surface_azimuth,
    const double astronomers_azimuth_angle
) {
    // Equation (47)
    ...
    const double I = r2d(
        acos(cos(zenit_rad)*cos(slope_rad) + sin(slope_rad)*sin(zenit_rad)*cos(astro_azimuth_rad - surf_azimuth_rad))
    );
...
auto solar_position(
    ...
    const double julian_ephemeris_day = jde(julian_day, DT);
    const double julian_ephemeris_centry = jce(julian_ephemeris_day);
    const double julian_ephemeris_millennium = jme(julian_ephemeris_centry);
    const double L = heliocentric_longitude(julian_ephemeris_millennium);
    const double B = heliocentric_latitude(julian_ephemeris_millennium);
    const double R = heliocentric_radius_vector(julian_ephemeris_millennium);
    const double beta = geocentric_latitude(B);
    const double Theta = geocentric_longitude(L);


    const auto [longitude_nutation, obliquity_nutation] = nutation(julian_ephemeris_centry);
    const double epsilon = true_ecliptic_obliquity(julian_ephemeris_millennium, obliquity_nutation);
    const double delta_tau = aberration_correction(R);
    const double lambda = apparent_sun_longitude(Theta, longitude_nutation, delta_tau);
    const double nu = apparent_Greenwich_sidereal_time(julian_day, julian_ephemeris_centry, longitude_nutation, epsilon);
    const double alpha = geocentric_sun_right_ascension(lambda, epsilon, beta);
    const double delta = geocentric_sun_declination(lambda, epsilon, beta);
    const double H = observer_local_hour_angle(nu, alpha, geographic_longitude);
    const auto [alpha_mark, delta_mark, H_mark] = topocentric_values(R, geographic_latitude, masl, alpha, delta, H);
    const double e0 = uncorrected_topocentric_elevation_angle(geographic_latitude, delta_mark, H_mark);
    const auto [Gamma, Phi] = topocentric_astronomers_and_azimuth_angle(H_mark, geographic_latitude, delta_mark);

    return collector(e0, Gamma, Phi, alpha_mark, delta_mark, H_mark);
}

inline std::pair<double, double> corrected_solar_elevation(const double e0, const double P, const double T) {
    return topocentric_zenith_angle(e0, P, T);
}

template<typename collector_t, typename dt_calc_t>
auto calendar_solar_position(
    unsigned int year,
    unsigned int month,
    double day,
    const double geographic_latitude,
    ...
    const auto DT = cal_calc(year, month, day);
    const double julian_day = jd_from_cal(year, month, day);
    return solar_position(julian_day, DT, geographic_latitude, geographic_longitude, masl, collector);
}

template<typename collector_t, typename dt_calc_t>
auto time_point_solar_position(
    ...
    const double julian_day = jd_from_clock(time_point);
