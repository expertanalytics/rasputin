...
def decode_dem(source: BinaryIO, *, nodata: float | None = None) -> DemTile:
    ...
    in the file's dtype, and must agree with tag 42113 when that is present.
    """
    with tifffile.TiffFile(source) as tif:
        page = _single_page(tif)
        dtype = _check_page(page)
        geokeys: dict[str, Any] = tif.geotiff_metadata or {}
        x_min, y_max, delta_x, delta_y, area = _placement(page, geokeys)
...
def _single_page(tif: tifffile.TiffFile) -> tifffile.TiffPage:
    ...
    for index in range(1, len(pages)):
        extra = pages[index]
        if isinstance(extra, tifffile.TiffPage) and not extra.is_reduced:
            raise GeoTiffError(
                f"NewSubfileType (254) = {int(extra.subfiletype)} on page {index} of "
                f"{len(pages)}: only page 0 may be full resolution"
            )
    return pages.first
...
def _check_page(page: tifffile.TiffPage) -> np.dtype[Any]:
    ...
    for name, code, value, known, enum in (
        ...
        if value not in known:
            scheme = enum(value).name if value in enum._value2member_map_ else "unknown"
            raise GeoTiffError(
                f"{name} ({code}) = {value} ({scheme}) cannot be decoded as installed; "
                "install the `codecs` extra (pip install 'rasputin[codecs]')"
            )
    return dtype


def _placement(
    page: tifffile.TiffPage, geokeys: dict[str, Any]
) -> tuple[float, float, float, float, bool]:
    """The node grid's origin and spacing (§4, §5 refusals 1-6)."""
    tags = page.tags
    if 34264 in tags:
    ...
    tie = [float(v) for v in tags[33922].value]
    if len(tie) > 6:
        raise GeoTiffError(f"ModelTiepointTag (33922) has {len(tie)} values: a GCP list")
    if len(tie) < 6 or not all(math.isfinite(v) for v in (*tie[:2], *tie[3:5])):
        raise GeoTiffError(f"ModelTiepointTag (33922) = {tie}: need finite I, J, X, Y")
    scale = [float(v) for v in tags[33550].value]
    if len(scale) != 3 or not all(math.isfinite(v) and v > 0 for v in scale[:2]):
...
def _nodata(
    ...
    if 42113 in page.tags:
        ...
        try:
            ...
            raise GeoTiffError(f"{where} is not a number") from None
