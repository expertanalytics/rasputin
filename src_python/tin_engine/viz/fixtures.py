...
def _fixture(
    ...
    chains: Sequence[tuple[Sequence[int], str, bool]],
) -> Fixture:
    ...
    for indices, role, is_river in chains:
        records.append(FixtureChain(len(flat), len(indices), role, is_river))
...
#: `river`, which differ only in the `is_river` bit -- written once so that the
...
CATCHMENT = _fixture(
    ...
    [
        (range(10), "outer", False),
        ([10, 11, 12, 13], "hole", False),
        ([14, 15, 16, 17], "hole", False),
        ([18, 19, 20], "breakline", False),
    ],
)
...
BREAKLINE_CHAIN = _fixture(
    ...
    [(range(4), "outer", False), ([4, 5, 6], "breakline", False)],
)

RIVER = _fixture(
    "river",
    "the same open breakline with is_river set, so that stroke is exercised before 5b needs it",
    _BOX_700_500 + _CROSSING,
    [(range(4), "outer", False), ([4, 5, 6], "breakline", True)],
)
